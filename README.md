...
титули, ресурси для збору, карти з точками появи) та всі зображення зібрані з онлайн-бази
знань по грі і запаковані в SQLite-бази, які йдуть разом із застосунком. Після
встановлення інтернет не потрібен.
...
* **Каталог** із деревом категорій (Зброя → Мечі, Доспіхи → Кожана броня → Верх …),
  фільтр за назвою, рівнем, якістю та расою.
...
```bash
# 0. Адреса онлайн-джерела задається змінною середовища (у PowerShell: $env:SOURCE_URL="https://…")
export SOURCE_URL=https://…

# 1. Зібрати дані (ru — основна мова джерела, en — англійська; є також de, fr, pl, tr, es)
...
Скрапер працює з JSON-API джерела (`/api/{item,npc,quest,skill,title,harvest}/{search,info}`,
...
| `categories` | дерево категорій для каталогу |
...
Ігрові дані та зображення належать NCSoft / Gameforge. Застосунок зроблено для
особистого некомерційного використання.
